...
void on_message_from_user1(const std::shared_ptr<vsomeip::message> &msg) {
...
void send_message(const std::string &text) {
    ...
    request->set_service(0x1111);
    request->set_instance(0x1000);
...
int main() {
    app = vsomeip::runtime::get()->create_application("chat_user2");
    app->init();

    app->register_state_handler([](vsomeip::state_type_e state) {
        if (state == vsomeip::state_type_e::ST_REGISTERED) {
            app->offer_service(0x2222, 0x2000);
            app->request_service(0x1111, 0x1000);
        }
    });

    app->register_message_handler(0x2222, 0x2000, 0x8888, on_message_from_user1);

    std::thread app_thread([] { app->start(); });

    std::cout << "\n🟢 ChatApp - Connected as User2" << std::endl;
